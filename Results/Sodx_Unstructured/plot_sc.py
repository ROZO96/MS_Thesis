...
case=sys.argv[1];
files=glob.glob(case+"/Bx_Scheme/sc_file_*")
fp=open(files[0])
lines=fp.readlines()
sc_Data=[];
times=[];
...
def animate(i):
	plt.clf() 
	plt.tripcolor(x_points, y_points,triangles, facecolors=np.asarray(Data[i]).T, vmax=1,edgecolors='k')
	plt.colorbar()
...
ani.save(case+'/Theta_evolution_Bx.gif', writer='pillow')
