...
def animate(i):
	...
	#theta_B_values=[]
	theta_Bx_values=[]
	n_triangle,time=int(lines_Bx[0].split()[0]),float(lines_Bx[0].split()[1])
	for j in range(np.size(lines_Bx)-1):
	...
	plt.hist(theta_Bx_values,bins=11,label=labels,weights=weight_Bx)
...
ani = animation.FuncAnimation(fig, animate,save_count=np.size(files_Bx))
